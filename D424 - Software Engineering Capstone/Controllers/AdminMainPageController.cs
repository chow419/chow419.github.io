...
namespace D424___Software_Engineering_Capstone.Controllers
{
    class AdminMainPageController : GuestMainPageController
    {
        ...
        public override VerticalStackLayout PopulateProfileMenuOverlay()
        {
            ...
            var adminPortalLabel = new Label()
            {
                Text = "Admin Portal",
                FontSize = 16,
                HorizontalOptions = LayoutOptions.End
            ...
            var adminPortalTapGesture = new TapGestureRecognizer();

            adminPortalTapGesture.Tapped += (s, e) =>
            {
                MainView.OnAdminPortalLabelTapped(s, e);
            };

            adminPortalLabel.GestureRecognizers.Add(adminPortalTapGesture);
